    ...
    case class Equations(substitutions: Map[V, P], equations: Seq[P], caseBashProgress: Seq[(Int, Int)]) {
      ...
      def addEquation(q: P): Option[Equations] = {
        ...
        def splitIfPositiveOrOtherwiseAdd: Option[Equations] = {
          if (p.size > 1 && (p.forall(_._2 > 0) || p.forall(_._2 < 0))) {
            addEquations(p.iterator.map(t => polynomialAlgebra.monomial(t._1)))
          } else {
            add
          }
        }
        ...
        def add: Some[Equations] = {
          if (p.size > 1) {
            require(p.exists(_._2 > 0) && p.exists(_._2 < 0))
          }
        ...
        p.totalDegree match {
          ...
          case Some(2) => {
            p.size match {
              ...
              case 2 => {
                val List(t1, t2) = p.toList.sortBy(_._1.values.sum)
              ...
              case 3 => {
                val List(t1, t2, t3) = p.toList.sortBy(t => (t._1.values.sum, t._2))
      ...
      def solveALinearEquation: Option[Equations] = {
        ...
        equations.find(p => p.totalDegree == Some(1) && p.termsOfDegree(1).exists(_._2 == 1) && p.count(_._2 > 0) == 1) match {
          case Some(p) => solve(p)
          case None => {
            equations.find(p => p.totalDegree == Some(1) && p.termsOfDegree(1).exists(_._2 == -1) && p.count(_._2 < 0) == 1) match {
