...
class S6SubgroupsTest extends FlatSpec with ShouldMatchers {

  "characters" should "give the right character table for (S_3 x S_3)_b \\subset S_6" in {
    val G = FiniteGroups.symmetricGroup(6).subgroupGeneratedBy(Set(IndexedSeq(1, 2, 0, 4, 5, 3), IndexedSeq(3, 5, 4, 0, 2, 1), IndexedSeq(3, 5, 4, 1, 0, 2), IndexedSeq(3, 4, 5, 0, 1, 2)))
