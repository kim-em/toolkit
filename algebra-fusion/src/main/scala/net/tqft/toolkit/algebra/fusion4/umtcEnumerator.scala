...
case class UMTCEnumerator(numberOfSelfDualObjects: Int, numberOfDualPairsOfObjects: Int, globalDimension: Double) {
  ...
  case class Eigenspace(eigenvalue: ComplexDouble, eigenbasis: Seq[Array[ComplexDouble]]) {
    lazy val projection = {
      Array.tabulate(eigenbasis.head.length, eigenbasis.head.length)({ (i,j) =>
        eigenbasis.map(v => v(i).mul(v(j).conj)).reduce(_.add(_))
      })
    }
  ...
  case class PartialFusionRing(partialRank: Int, matrices: Array[Array[Array[Int]]], eigenspaces: Seq[Eigenspace], equations: Nothing) {
    require(matrices.size == partialRank - 1)
