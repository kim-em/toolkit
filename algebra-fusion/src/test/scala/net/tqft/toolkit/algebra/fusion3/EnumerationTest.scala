...
//    r.size should equal(1)
//  }
  "Enumeration" should "find all the rank 8 self dual unitary MTCs with global dimension at most 40" in {
    val r = results(Enumeration(8,0, 40, umtc = true, None, None,None))
   for(ring <- r) {
      println(ring)
    } 
  }
