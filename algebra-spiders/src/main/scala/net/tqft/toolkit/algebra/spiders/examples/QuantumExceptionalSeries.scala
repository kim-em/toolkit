...
//      reducedDiagrams(4, Map(VertexType(3, 0, 1) -> 2)) ++
//      reducedDiagrams(4, Map(VertexType(4, 0, 2) -> 1)).headOption).ensuring(_.size == 5))
//
//  lazy val basisFor5Boxes = {
//    val tangle = reducedDiagrams(5, Map(VertexType(3, 0, 1) -> 3, VertexType(4, 0, 2) -> 1)).head
//    val tangles = Seq.tabulate(5)(k => diagramSpider.rotate(tangle, k))
//
//    basis(5, (reducedDiagrams(5, Map(VertexType(3, 0, 1) -> 1)) ++
//      reducedDiagrams(5, Map(VertexType(3,0, 1) -> 3)) ++
//      reducedDiagrams(5, Map(VertexType(3, 0,1) -> 5)) ++ tangles).ensuring(_.size == 16))
//  }
//
