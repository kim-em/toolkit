...
object BoundaryConnectedPlanarGraphs {
  ...
  def trivalent(n: Int, k: Int) = {
    def generator(r: Int, s: Int) = if (r > 2)
      ConnectedTrivalentPlanarGraphs(r, s).flatMap((G: PlanarGraph) => Seq.tabulate(r)((rotation: Int) => DiagramSpider.graphSpider.rotate(G, rotation).canonicalFormWithDefect._1)).distinct
