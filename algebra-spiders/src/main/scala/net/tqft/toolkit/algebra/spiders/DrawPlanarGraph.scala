...
trait DrawPlanarGraph {
  ...
  def apply(G: PlanarGraph, crossings: Seq[Int] = Seq.empty): String = {
    ...
    if (crossings.nonEmpty) {
      crossings.map((v) => assert(G.degree(v) == 4))
      // Decorate vertices as crossings
      decoratedVertices = crossings.map((v) => (v, "minimum size=0pt")).toMap
      // Decorate crossing edges
      var undercrossingEdges = Seq[(Int, Int, Int)]() // (edge, vertex, index) means "edge" undercrosses at the endpoint "vertex".
      // "index" is used to distinguish between positive and negative loops.
      for (vertex <- crossings; i <- List(1, 3)) {
        undercrossingEdges = (cyclicReverse(G.edgesAdjacentTo(vertex))(i), vertex, i) +: undercrossingEdges
      } // 1th and 3th edges are the under-crossings

      for ((edge, endpoint, i) <- undercrossingEdges) {
        val startVertex = G.edgeVertexIncidences(edge)._1
        val endVertex = G.edgeVertexIncidences(edge)._2

        val styleStr =
          if (startVertex == endVertex) { // Edge is a loop
            if (i == 1) {
              if (G.edgesAdjacentTo(endpoint).head == edge) "shorten >=2.5pt" // Inward undercrossing of negative twist
              else "shorten <=2.5pt" // Outward undercrossing, positive twist
            } else {
              if (G.edgesAdjacentTo(endpoint).head == edge) "shorten >=2.5pt" // Inward undercrossing, positive twist
              else "shorten >=2.5pt" // Outward undercrossing, negative twist
            }
          } else { // Edge is not a loop
            if (startVertex == 0 || endVertex == 0) "shorten <=2.5pt" // Boundary-connected edge; in "draw" the boundary vertex is always the end vertex, so shorten edge at the start
            else if (endpoint == startVertex) "shorten <=2.5pt" else "shorten >=2.5pt" // Internal edge
          }
  ...
  def draw(G: PlanarGraph, decoratedEdges: Map[Int, String], decoratedVertices: Map[Int, String], hideBoundary: Boolean): String = {
  ...
  def pdf(pdfPath: String, g: PlanarGraph, crossings: Seq[Int] = Seq.empty) {
    pdfMultiple(pdfPath, Seq(g), Seq(crossings))
  }

  def pdfMultiple(pdfPath: String, Gs: Seq[PlanarGraph], crossings: Seq[Seq[Int]] = Seq.empty): Unit = {
    // Writes TikZ to tex file and runs pdflatex
    val processedCrossings = if (crossings.isEmpty) for (i <- 0 until Gs.length) yield Seq[Int]() else crossings // Hacky! :\
    val outputStr = (Gs zip processedCrossings).map((t: (PlanarGraph, Seq[Int])) => DrawPlanarGraph(t._1, t._2)).mkString(
