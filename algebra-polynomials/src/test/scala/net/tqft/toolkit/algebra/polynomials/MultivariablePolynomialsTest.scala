...
class MultivariablePolynomialsTest extends FlatSpec with Matchers {
  ...
  "gcd" should "not overflow badly" in {
    val polynomials = implicitly[MultivariablePolynomialAlgebraOverEuclideanRing[Int, String]]
    val b = "b"
    val d = "d"
    val p = MultivariablePolynomial[Int, String](Map(Map(d -> 5) -> -4, Map(d -> 1) -> -2, Map() -> -1, Map(d -> 7) -> 1, Map(d -> 3) -> 5, Map(d -> 4) -> -1, Map(d -> 2) -> 2))
    val q = MultivariablePolynomial[Int, String](Map(Map(b -> 4, d -> 2) -> 2, Map(b -> 4, d -> 3) -> -3, Map(b -> 4, d -> 6) -> 3, Map(b -> 4, d -> 5) -> 5, Map(b -> 4, d -> 7) -> -4, Map(b -> 4, d -> 1) -> 1, Map(b -> 4, d -> 4) -> -5, Map(b -> 4, d -> 8) -> 1))
    val g = polynomials.gcd(p, q)
    require(g.coefficients.valuesIterator.forall(x => x > -100 && x < 100))
  }
